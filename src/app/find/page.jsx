...
useEffect(() => {
  const fetchGeolocation = async () => {
    if (error) {
      ...
      console.log(data);
      setPosition({
        latitude: data.latitude == null ? 53.3803991 : data.latitude,
...
const [locationData, setLocationData] = useState(null);
      ...
      const [center, setCenter] = useState({ lat: position.latitude??53.3803991, lng: position.longitude??-1.4885347});
  ...
  return (<><ManagerContext.Provider value={{ position, error }}>
    <div className="mx-[20px] mt-8">
      ...
      <MyGoogleMap center={center} onClick={handleMapClick} routelat={routelat.lat} routelng={routelat.lng} />
      {location && (
        <div>
    ...
    return data.map((element, index) => (
      <div key={index}  onClick={() => setRoutelat({lat:element.location.latitude, lng:element.location.longitude})}>
        {<Hotspot imageSrc={element.image}  subtitle={element.description} title={element.name} 
          origin={JSON.stringify({lat:position.latitude, lng:position.longitude})} 
